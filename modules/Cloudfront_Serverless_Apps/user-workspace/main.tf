...
resource "aws_cloudfront_distribution" "s3_distribution" {
  ...
  origin {
    ...
    origin_id   = "origin-s3-${var.name}-${var.stage}"
  ...
  origin {
    ...
    origin_id   = "origin-api-${var.name}-${var.stage}"
  ...
  origin {
    domain_name = element(split("/", "${data.aws_cloudformation_stack.network.outputs[var.cloudformation_stack_output_endpoint_variable_GraphQL]}"),2)
    origin_id   = "origin-graphql-${var.name}-${var.stage}"
    origin_path = format("/%s",element(split("/", "${data.aws_cloudformation_stack.network.outputs[var.cloudformation_stack_output_endpoint_variable_GraphQL]}"),3))
  ...
  default_cache_behavior {
    ...
    target_origin_id       = "origin-api-${var.name}-${var.stage}"
  ...
  ordered_cache_behavior {
    ...
    smooth_streaming       = false
    target_origin_id       = "origin-s3-${var.name}-${var.stage}"
    viewer_protocol_policy = var.ordered_cache_behavior_viewer_protocol_policy
  }
  ...
  ordered_cache_behavior {
    ...
    max_ttl                = var.ordered_cache_behavior_max_ttl
    min_ttl                = var.ordered_cache_behavior_min_ttl
    path_pattern           = "workspace/graphql"
    smooth_streaming       = false
    target_origin_id       = "origin-graphql-${var.name}-${var.stage}"
    viewer_protocol_policy = var.ordered_cache_behavior_viewer_protocol_policy
  }
  ...
  ordered_cache_behavior {
    ...
    #cache_policy_id        = aws_cloudfront_cache_policy.redirect_lambda.id
    cache_policy_id        = aws_cloudfront_cache_policy.standard.id
    max_ttl                = var.ordered_cache_behavior_max_ttl
    min_ttl                = var.ordered_cache_behavior_min_ttl
