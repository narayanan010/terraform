...
resource "aws_cloudfront_distribution" "s3_distribution" {
  ...
  ordered_cache_behavior {
    ...
    cached_methods  = ["GET", "HEAD"]
    compress        = true
    default_ttl     = var.storybook_ordered_cache_behavior_default_ttl
    ...
    max_ttl                = var.storybook_ordered_cache_behavior_max_ttl
    min_ttl                = var.storybook_ordered_cache_behavior_min_ttl
    path_pattern           = var.s3_path_pattern_storybook
    smooth_streaming       = false
    target_origin_id       = "origin-s3-${var.namespace}-${var.name}-${var.vertical}-${var.stage}"
    viewer_protocol_policy = var.ordered_cache_behavior_viewer_protocol_policy
  }
  ...
  ordered_cache_behavior {
    ...
    max_ttl                = var.next_ordered_cache_behavior_max_ttl
    min_ttl                = var.next_ordered_cache_behavior_min_ttl
    path_pattern           = var.s3_path_pattern_next
    smooth_streaming       = false
    target_origin_id       = "origin-s3-${var.namespace}-${var.name}-${var.vertical}-${var.stage}"
    viewer_protocol_policy = var.ordered_cache_behavior_viewer_protocol_policy
  }
...
data "aws_iam_policy_document" "s3_policy_primary_bucket" {
  ...
  statement { # For OAC 
    ...
    resources = ["${data.aws_s3_bucket.cf_primary_bucket.arn}/*"]
