...
data "aws_iam_policy_document" "github_actions_services_ui" {
  ...
  statement {
    ...
    resources = [
      "arn:aws:eks:${data.aws_region.current.name}:${data.aws_caller_identity.current.account_id}:cluster/capterra-${var.environment}-eks",
      "arn:aws:eks:${data.aws_region.current.name}:${data.aws_caller_identity.current.account_id}:cluster/capterra-${var.environment}-eks/*/*"
    ]
  }
  ...
  statement {
    ...
    resources = ["arn:aws:kms:${data.aws_region.current.name}:${data.aws_caller_identity.current.account_id}:key/5c8ab0dc-f8f5-4760-8b42-7c83fc005f48"]
  }
