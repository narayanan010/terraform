resource "datadog_monitor" "msk-offline-replicas" {
  name               = "[Production-DR] Offline Replicas for ClickStream MSK"
  type               = "metric alert"
  ...
  query = "min(last_10m):sum:aws.msk.kafka.server.replica_manager.OfflineReplicaCount{${aws_msk_cluster.click_streaming_dr.arn}} > 1"
...
resource "datadog_monitor" "msk-consumer-lag" {
  name = "[Production-DR] ClickStream consumer lag is high"
  type = "metric alert"
  # @slack-alert_click_streaming_stg
  message            = "${var.alerts_to} Consumer group for Production-DR click stream is lagging, please check the stream readers."
  ...
  query = "avg(last_5m):sum:aws.msk.kafka.consumer.group.ConsumerLagMetrics.Value{${aws_msk_cluster.click_streaming_dr.arn}} by {cluster_arn, topic,groupid} > 1800"
...
resource "datadog_monitor" "msk-low-data" {
  name               = "[Production-DR] Unexpectedly low clickstream volume"
  type               = "metric alert"
  message            = "${var.alerts_to} Click volue in Production-DR click stream is low, please check the stream readers."
...
resource "datadog_monitor" "msk-under-replicated-partitions" {
  name               = "[Production-DR] Partitions are under-replicated in ClickStream MSK"
  type               = "metric alert"
...
resource "datadog_monitor" "msk-storage-space" {
  name    = "[Production-DR] Storage space is unexpectedly low in MSK"
  type    = "metric alert"
  ...
  query            = "avg(last_15m):min:aws.msk.node.filesystem.free.bytes{${aws_msk_cluster.click_streaming_dr.arn}} by {broker_id} < 20000000000"
  restricted_roles = [data.datadog_role.dg_admin.id]
