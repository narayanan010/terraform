resource "aws_cloudwatch_log_group" "declared_data" {
  name              = "/aws/msk/declared-data-prod-ue1"
  retention_in_days = 7
}


resource "aws_msk_cluster" "declared_data" {
  cluster_name           = "capterra-shared-data-msk-prod-ue1"
  kafka_version          = "2.8.1"
  ...
  broker_node_group_info {
    ...
    ebs_volume_size = 500
    client_subnets  = ["subnet-0231e00693814e9ae", "subnet-05d54ce5511ff83cc"]
    security_groups = [
...
resource "aws_secretsmanager_secret" "declared_data" {
  name       = "AmazonMSK_declared_data_msk_prod"
