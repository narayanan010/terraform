locals {
  ...
  eks_environments = ["dev", "staging", "production"]
}
...
data "aws_iam_policy_document" "eks_main_roles_policy" {
  ...
  statement {
    ...
    resources = [
      "arn:aws:eks:${data.aws_region.current.name}:${data.aws_caller_identity.current.account_id}:cluster/capterra-${each.key}-eks"
    ]
  }
