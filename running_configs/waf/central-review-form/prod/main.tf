module "aws_waf_module" {
  ...
  custom_path_size_checker_action_list = [
    ...
    { uri_path = "/k8s/api/v1/", uri_path_constraint = "EXACTLY", size = 8192, size_comparison_operator = "GE", action = "allow", priority = 4 }
  ]

# Custom rule group
  custom_rule_group_list = [
    {rule_group_arn = aws_wafv2_rule_group.method_regex.arn, priority = 5},
    {rule_group_arn = aws_wafv2_rule_group.spammer.arn, priority = 12}
  ]

  # Oversized requests
  oversized_body_requests_rule_status             = "block"
  oversized_body_requests_rule_priority           = 6
  oversized_headers_requests_rule_status          = "block"
  oversized_headers_requests_rule_priority        = 7
  oversized_cookies_requests_rule_status          = "block"
  oversized_cookies_requests_rule_priority        = 8

  # DENY LIST rule
  custom_deny_ip_list_status                      = "block"
  custom_deny_ip_list_rule_priority               = 9
  ...
  aws_managed_rules_ip_reputation_list_priority   = 10

  # AWS KNOWN BAD INPUTS rule set
  aws_managed_rules_known_bad_inputs_set_enabled  = true
  aws_managed_rules_known_bad_inputs_set_priority = 11

  # AWS CORE rule set
  aws_managed_rules_common_rule_set_enabled       = true
  aws_managed_rules_common_rule_set_priority      = 13

  # Nginix rule
  nginx_server_list_rule_priority                 = 14
