resource "aws_wafv2_rule_group" "custom" {
  ...
  name     = "${var.vertical}-${var.application}-${var.stage}-custom"
  scope    = var.web_acl_scope
  capacity = 2

  rule {
    name     = "test-header-rule-2"
    priority = 0
    ...
    statement {
      byte_match_statement {
        search_string = "X-HTTP-Method-Override"
        field_to_match {
          headers {
            match_pattern {
              all {}
    ...
    visibility_config {
      ...
      metric_name                = "test-header-rule-2"
  ...
  visibility_config {
    cloudwatch_metrics_enabled = true
    metric_name                = "${var.vertical}-${var.application}-${var.stage}-custom"
    sampled_requests_enabled   = true
  }
}
