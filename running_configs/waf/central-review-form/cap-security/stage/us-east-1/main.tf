module "aws_waf_module" {
  ...
  custom_path_size_checker_action_list = [
    ...
    { uri_path = "/partialreview", uri_path_constraint = "ENDS_WITH", size = 8192, size_comparison_operator = "GE", action = "allow", priority = 1 },
    { uri_path = "/review", uri_path_constraint = "ENDS_WITH", size = 8192, size_comparison_operator = "GE", action = "allow", priority = 2 },
    { uri_path = "/honeypot", uri_path_constraint = "ENDS_WITH", size = 8192, size_comparison_operator = "GE", action = "allow", priority = 3 }
  ]

  # Oversized requests
  oversized_body_requests_rule_status             = "block"
  oversized_body_requests_rule_priority           = 4
  oversized_headers_requests_rule_status          = "block"
  oversized_headers_requests_rule_priority        = 5
  oversized_cookies_requests_rule_status          = "block"
  oversized_cookies_requests_rule_priority        = 6

  # DENY LIST rule
  custom_deny_ip_list_status                      = "block"
  custom_deny_ip_list_rule_priority               = 7
  ...
  aws_managed_rules_ip_reputation_list_priority   = 8

  # AWS KNOWN BAD INPUTS rule set
  aws_managed_rules_known_bad_inputs_set_enabled  = true
  aws_managed_rules_known_bad_inputs_set_priority = 9

  # AWS CORE rule set
  aws_managed_rules_common_rule_set_enabled       = true
  aws_managed_rules_common_rule_set_priority      = 10

  # Nginix rule
  nginx_server_list_rule_priority                 = 11
