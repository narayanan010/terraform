provider "aws" {
  ...
  assume_role {
    #role_arn = lookup(var.modulecaller_assume_role_deployer_account, terraform.workspace)
    role_arn = "${var.modulecaller_assume_role_deployer_account[terraform.workspace]}"
  }
  ...
  ignore_tags {
    ...
    keys         = ["CreatorAutoTag", "CreatorId", "last_update", "runner_info","git_url","git_path"]
  }
}
